...
function TextBody({ data, onChange }: { data: TextPanelData; onChange: (d: TextPanelData) => void }) {
  return (
    <textarea
      ...
      placeholder="Type here to add notes, backstories, and anything else you need..."
      rows={5}
      ...
      {data.attributes.map((attr, i) => (
        <div key={i} className="flex items-center gap-2 group">
          <input
            ...
            className="text-sm font-semibold text-neutral-200 bg-transparent outline-none w-36 shrink-0 truncate focus:text-orange-400"
          />
          ...
          <input
            ...
            placeholder="—"
            className="text-sm text-neutral-400 bg-transparent outline-none flex-1 min-w-0"
          />
      ...
      {data.items.map((item, i) => (
        <div key={i} className="flex items-center gap-2 group">
          ...
          <input
            ...
            className="text-sm text-neutral-300 bg-transparent outline-none flex-1"
          />
      ...
      {data.stats.map((s, i) => (
        <div key={i} className="group relative bg-neutral-800/50 rounded-md p-2">
          <input
            value={s.value}
            onChange={(e) => update(i, 'value', e.target.value)}
            className="text-lg font-semibold text-orange-400 bg-transparent outline-none w-full"
          />
          <input
            value={s.label}
            onChange={(e) => update(i, 'label', e.target.value)}
            ...
            {data.columns.map((col, i) => (
              <th key={i} className="text-left border-b border-neutral-800 pb-1.5">
                <input
                  ...
                  className="bg-transparent text-xs uppercase tracking-wide text-neutral-500 outline-none w-full"
                />
              </th>
            ))}
            <th className="w-6" />
...
function ImageBody({ data, onChange }: { data: ImagePanelData; onChange: (d: ImagePanelData) => void }) {
  return (
    <div className="space-y-2">
      {data.imageUrl && (
        <img src={data.imageUrl} alt={data.caption} className="w-full rounded-md border border-neutral-800 object-cover max-h-64" />
      )}
      <input
        value={data.imageUrl}
        onChange={(e) => onChange({ ...data, imageUrl: e.target.value })}
        placeholder="Paste an image URL or local file path..."
        className="w-full text-xs bg-neutral-800/50 border border-neutral-800 rounded px-2 py-1.5 text-neutral-300 outline-none focus:border-orange-600"
      />
      <input
        value={data.caption}
