...
ipcMain.handle("vault:list-entries", async (_event, vaultPath, moduleFolder) => {
	const dirPath = path.join(vaultPath, moduleFolder);
	try {
		await fs.mkdir(dirPath, { recursive: true });
		return (await fs.readdir(dirPath)).filter((f) => f.endsWith(".md"));
	} catch (err) {
		console.error("list-entries failed:", err);
		return [];
	}
});
ipcMain.handle("vault:read-entry", async (_event, vaultPath, moduleFolder, fileName) => {
...
ipcMain.handle("vault:write-entry", async (_event, vaultPath, moduleFolder, fileName, data, content) => {
	const dirPath = path.join(vaultPath, moduleFolder);
	await fs.mkdir(dirPath, { recursive: true });
